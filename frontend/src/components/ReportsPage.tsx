...
export function ReportsPage({ investigationName = 'Active Investigation' }: { investigationName?: string }) {
  return (
    <div className="standard-page">
      ...
      <div className="export-grid">{formats.map(({title,description,format,icon:Icon,href,filename})=><article className="panel export-card" key={title}><span><Icon size={20}/></span><div><h3>{title}</h3><p>{description}</p></div><a href={href} onClick={(event)=>{event.preventDefault();void api.download(href,filename)}}><em>{format}</em><Download size={15}/></a></article>)}</div>
